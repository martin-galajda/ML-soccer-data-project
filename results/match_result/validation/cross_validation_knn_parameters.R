...
results <- data.frame(x.axis, CV.error.knn.from.1.to.400)

min.idx <- which(CV.error.knn.from.1.to.400 == min(CV.error.knn.from.1.to.400))

ggplot(data = results, aes(x = x.axis)) + 
  theme_bw() + 
  geom_line(aes(y = CV.error.knn.from.1.to.400)) +
  geom_point(aes(x = x.axis[min.idx], y = CV.error.knn.from.1.to.400[min.idx], colour = "red" )) +
